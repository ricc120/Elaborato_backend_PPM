...
| **Regular** | `userBasic` | `us3r1password` | Read-only weather queries (Rate-limited to 5 per day). |
...
**1.1 Get Tokens (Login as Regular User)**
```bash
...
If you run this 6 times without a token (or with a basic user token), you will receive a `429 Too Many Requests` status.
```bash
...
```bash
http POST https://ricc120.pythonanywhere.com/api/favorites/ name="Milan" "Authorization: Bearer <TOKEN>"
```
...
```
**2.4 Check Aggregated Statistics**
...
**4.1 Regular User attempting Editor Actions (Expect 403 Forbidden)**
